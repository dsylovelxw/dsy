layui.use(['form','layer','layedit','laydate','upload'],function(){
  ...
  /*   --- 验证权限名是否存在 ---------------------------------------------------------------
    ...
    form.on('submit(menuUser)',function(data){
		...
		$.post("/denglu/QuanSerlvet?action=addMenu&mname="+mname+"&mfunction="+mfunction+"&type2="+type2+"&icon="+icon+"&mbtn="+mbtn+"&father="+father,function(data){
			var info = eval("("+data+")");
			alert(info)
	 		if(info == 1){
    ...
    form.on('select(type2)', function(data){
        ...
        if(type == 3){
        	...
        	$.get("/denglu/QuanSerlvet?action=allButtenType",function (res) {
