layui.extend({
    ...
    var tableIns = table.render({
        elem: '#demmmm',
        url : '/denglu/QuanSerlvet?action=EasybuyUserList',
        toolbar: '#toolbarDemo',
        page : true,
        height: 'full-145',
        limit : 10,
        ...
        cols : [[
            ...
            {field: 'identityCode', title: '身份证号',  align:'center'},
	  ...
	  //修改用户
	    function upUser(userid){
	    	layui.layer.open({
	    		...
	    		success:function(layero, index){
	    			$.ajax({
	    				url:"/denglu/UserSerlvet?action=allUserByUserid",
	    				type:"post",
	    				data:{"userid":userid},
	    				success:function(data){
	    					...
	    					body.find("#uname2").val(info.loginName);
	    					...
	    					$.ajax({
	    						  ...
	    						  success:function(data){
	    							  ...
	    							  $.ajax({
	    								  url:"/denglu/LodeSerlvet?action=queryUserIsRole",	//查询用户是否有角色有返回1,没有返回0
	    								  data:{"userid":userid},
	    								  type:"post",
	    								  success:function(data){
	    									  if(data == 0){
	    ...
	    function addUser(){
	    	layui.layer.open({
	    		title : "添加用户",
	    		type : 2,
	    		content : "jsp/yonghu/userAdd.jsp",
	    		area:['400px','490px'],
	    		success:function(layero, index){
	    			/*------下拉框赋值--------*/
					$.ajax({
